...
const Navbar = ({ setSearchTerm }) => {
  ...
  const totalItems = useMemo(() => {
    if (!Array.isArray(cart.items)) return 0; // Ensure cart.items is an array
    let quantity = 0;
    for (let i = 0; i < cart.items.length; i++) {
      // Check if quantity is a valid number
      quantity += isNaN(cart.items[i].quantity) ? 0 : cart.items[i].quantity;
    }
    return quantity;
  }, [cart]);

  return (
    <div className="w-full flex flex-wrap bg-[#131921] text-white font-roboto py-2 px-3 items-center z-10 relative justify-between">
      {/* Left Logo and Address - Hidden on screens <= 420px */}
      <div className="hidden sm:flex items-center gap-3">
        ...
        <div className="flex w-fit">
          ...
          <div className="mt-2 flex flex-col ml-2">
            <div className="text-[#CCCCCC] text-sm">Delivering to Moradabad</div>
      ...
      {/* Nav Links - Hidden on screens <= 420px */}
      <ul className="hidden sm:flex gap-8 text-lg items-center">
        ...
        <li className="text-sm">
          <p className="text-sm/3" onClick={() => navigate(`/login`)}>Hello, {username}</p>
          <div>
            <p className="flex font-bold cursor-pointer" onClick={() => setShowModal(true)}>
              Accounts & Lists
              <span><img src="/dropdown.png" alt="dropdown" /></span>
            </p>
